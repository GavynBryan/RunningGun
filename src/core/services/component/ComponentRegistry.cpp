...
bool ComponentRegistry::Contains(std::string_view typeName) const
{
	return m_Registry.find(std::string(typeName)) != m_Registry.end();
}

std::unique_ptr<ActorComponent> ComponentRegistry::Create(
...
std::unique_ptr<ActorComponent> ComponentRegistry::CreateFromJson(
	std::string_view typeName,
	Actor& actor,
...
const std::vector<const ComponentMeta*>& ComponentRegistry::All() const
{
